...
void display1(void)
{
    ...
    display_string(190, 900, "SCENERY CLIPPER", 1);
    ...
    display_string(500, 650, "Aastha(1dt19cs001) & Ankita Nath(1dt19cs019)", 2);
    glutPostRedisplay();
    glutSwapBuffers();
}
...
void edgedetect(GLfloat x1, GLfloat y1, GLfloat x2, GLfloat y2, int *le, int *re)
{
    float mx, x, temp;
    int i, k;
    if ((y2 - y1) < 0) //swap sp dont get -ve value
    {
        temp = x1;
        x1 = x2;
        x2 = temp;
    ...
    if ((y2 - y1) != 0) 
        ...
        mx = x2 - x1;
    x = x1;
    for (i = (int)y1 + 1; i < (int)y2; i++) //follow along the edge of polygon
    {
        int t = 0;
        if (x < (float)le[i])
        {
...
int main(int argc, char **argv)
{
    ...
    glutCreateWindow("Scenery Clipping"); // creates the window with name line
    init();
